...
import type {
  ...
  SlackbotV2ResolvedOptions,
  SlackbotV2ThreadState,
  SlackbotV2Trace,
} from "./types";

const SLACK_PLATFORM: SessionPlatform = {
  source: "slackbotv2",
  platform: "slack",
  attachmentLabel: "Slack attachment",
  tracePrefix: "slackbotv2",
  apiKeyEnvVars: ["SLACKBOT_API_KEY", "CENTAUR_API_KEY"],
};
...
export function createSlackbotV2(rawOptions: SlackbotV2Options): SlackbotV2 {
  const options: SlackbotV2ResolvedOptions = {
    ...rawOptions,
    platform: rawOptions.platform ?? SLACK_PLATFORM,
  };
...
async function syncThreadMessageToSession(
  ...
  input: {
    ...
    options: SlackbotV2ResolvedOptions;
  },
): Promise<void> {
  ...
  try {
    ...
    traceLog(
      input.options,
      "slackbotv2_forward_active_execution_marked",
      trace,
    );
...
async function setAssistantTitle(
  ...
  await ignoreAssistantError(() =>
    adapter.setAssistantTitle!(
      target.channel,
      target.threadTs,
      clipOneLine(normalized, 80),
    ),
  );
}
