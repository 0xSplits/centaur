import { isJsonObject, stringValue } from "@centaur/chat-session-bridge";
...
export function isAllowedSlackWebhookBody(
  ...
  if (!isRawSlackEnvelope(payload) || payload.type !== "event_callback")
    return true;
  const event = isRawSlackEvent(payload.event) ? payload.event : undefined;
  if (!event) return true;

  const allowedExternalTeamIds =
    options.allowedExternalTeamIds ??
    splitEnvList(process.env.SLACKBOT_EXTERNAL_ORG_ALLOWLIST);
  const externalTeamId = externalSlackTeamIdForHome(
    stringValue(payload.team_id),
    event,
  );
...
export function isAllowedSlackMessage(
  ...
  const triggerBotAllowlist =
    ...
    splitEnvList(process.env.SLACKBOT_TRIGGER_BOT_ALLOWLIST);
  const botAuthored =
    message.author.isBot === true ||
    (raw ? isBotAuthoredSlackEvent(raw) : false);
  if (
    botAuthored &&
    !(raw && isAllowedTriggerBotMessage(raw, triggerBotAllowlist))
  ) {
...
function isAllowedTriggerBotMessage(
  ...
  const botIds = normalizedIdentifierSet(
    ...
    stringValue(event.bot_profile?.id),
  );
...
function isRawSlackEvent(value: unknown): value is RawSlackEvent {
  return (
    isJsonObject(value) &&
    (value.bot_profile === undefined || isJsonObject(value.bot_profile))
  );
}
