...
| `CODEX_BEDROCK_REGION` | `sandbox.extraEnv`. | Opt-in switch for codex's `amazon-bedrock` provider. When set, the control plane registers the AWS SigV4 re-signing credential (scoped to the `bedrock` service and this region, upstream `bedrock-mantle.<region>.api.aws`) and injects the placeholder `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY` env so codex can sign requests iron-proxy re-signs with the real IAM keys. Unset disables Bedrock; the provider's own default region is `us-east-1`. See [Codex with Amazon Bedrock](/deploying-in-production#codex-with-amazon-bedrock). |
